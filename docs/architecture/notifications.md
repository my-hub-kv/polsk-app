...
The initial activity schedule uses this foundation on creation only: it creates one in-app
notification for every login account participating in the same event year, with a stable
per-activity idempotency key and an internal activity-detail destination. Activity edits do
not currently notify.
