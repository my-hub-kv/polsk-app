...
- Every adult and child has a historical participant profile; login credentials are separate, use username/password, and email is optional for invitation or recovery. Administrators can reset credentials; invitations may use links, QR codes, or optional email.
- Adults may act as children in the same household only. Audited actions record the acting account and active profile. Exact birth dates/years are not stored; age groups are 0–3, 4–11, 12–18, and adult.
...
- All participants may create activities; administrators manage all activities. External calendar export is not required. Calendar reminders are approximately 15 minutes before an activity.
- Chores are assigned to individuals, have exactly one `A` task-responsible person, and use `X` adult, `B` child, and dinner-only `BA` child-food-responsible roles. Dinner `A` and `BA` must differ. Each assignment counts as one chore; there is no completion workflow.
- Planning preserves sticky/locked assignments, respects attendance, daily availability, workload, eligibility, and household child/adult pairing. Plans are Draft, Published, or Archived; participants see only Published plans. Day responsibility is derived from the next day’s dinner `A` and does not add a chore count.
...
- Communication is channel-based: public channels are discoverable and opt-in for notifications; official channels are administrator-designated, universally subscribed, and not mutable; private channels are administrator-created and member-only. No direct messages or read acknowledgements in v1.
...
- Any participant may request shopping; a coordinator reviews, edits, merges, and places requests on delivery-day lists. Explainable, editable suggestions use stock, reservations, attendance, historical context, and weather. Lists have operational states and safe CSV exports with history/change-after-export indication.
...
- In-app notifications, provider-neutral push, and invitation/credential email are supported concepts. Public-channel notifications default off; official channels cannot be muted; draft chore-plan changes never notify participants; delivery should be idempotent.
