...
- In-app notifications and provider-neutral push are supported. The notification badge clears when a participant opens the notification center. Public-channel notifications default off; official channels cannot be muted; draft chore-plan changes never notify participants. In-app enqueueing is idempotent per recipient and event year; configured push delivery is synchronous after the originating transaction commits by default, while a future scheduler may be deliberately enabled instead. Administrators have event-scoped in-app controls for pending notification delivery and expired throttle cleanup. Ambiguous external push transport failures are not retried, and the in-app inbox remains authoritative.
